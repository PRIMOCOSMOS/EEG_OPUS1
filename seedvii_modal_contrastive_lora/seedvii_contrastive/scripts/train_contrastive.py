...
def _find_local_llm_dir(base: Path, preferred_name: str = "") -> Path | None:
    """Find an actual Transformers model directory under ModelScope cache."""
    if not base.exists():
        return None
...
def build_models(cfg, device):
    ...
    text = LoRATextTower(
        ...
        torch_dtype=torch.float32,  # FIX: Use float32 for stability
    ).to(device)
...
def evaluate(eeg, text, loader, device, text_csv_path):
    """Evaluate EEG encoder using L2-text prototypes."""
    eeg.eval()
    text.eval()
    
    # Build text embeddings bank
    bank_texts, bank_labels, _ = build_l2_text_bank(text_csv_path)
    bank_z = text(bank_texts)
    
    # Ensure float32 for consistency
    bank_z = bank_z.to(dtype=torch.float32)
...
def main():
    ap = argparse.ArgumentParser(description="Train EEG-LLM contrastive model")
    ...
    if tcfg.get("resume", True) and ckpt_path.exists():
        try:
            ...
            print(f"[Train] WARNING: failed to resume checkpoint: {e}")
    ...
    for epoch in range(start_epoch, tcfg["epochs"]):
        ...
        for batch_idx, indices in enumerate(pbar):
            ...
            # Loss (dtype handling is done inside criterion)
