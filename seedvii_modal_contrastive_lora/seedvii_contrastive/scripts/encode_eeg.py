"""Encode EEG windows using a trained checkpoint (v3 MoCo compatible)."""
...
from seedvii_contrastive.data.dataset import (
    load_index, split_index_by_subjects, WindowNpzDataset, build_l2_text_bank,
)
...
def main():
    ...
    # Build models (v3 returns 4-tuple: eeg, eeg_momentum, eeg_queue, text)
    eeg, eeg_momentum, eeg_queue, text = build_models(cfg, device, train_llm=False)
    load_ckpt(args.checkpoint, eeg, eeg_momentum, eeg_queue, text, device=device)
    eeg.eval()
    text.eval()

    # Data
    df = load_index(cfg["data"]["npz_dir"])
    tr_df, va_df = split_index_by_subjects(
        df, cfg["data"]["train_subjects"], cfg["data"]["val_subjects"])
    validate_training_split(df, tr_df, va_df,
                            cfg["data"]["train_subjects"], cfg["data"]["val_subjects"])
    use_df = {"train": tr_df, "val": va_df, "all": df}[args.split].reset_index(drop=True)
    if len(use_df) == 0:
        raise ValueError(f"split {args.split!r} has zero windows")

    stats_path = Path(cfg["runtime"]["output_dir"]) / "norm_stats.npz"
    if not stats_path.exists():
        raise FileNotFoundError(f"norm stats not found: {stats_path}")
    stats = np.load(stats_path)
    ds = WindowNpzDataset(use_df, stats["mean"], stats["std"],
