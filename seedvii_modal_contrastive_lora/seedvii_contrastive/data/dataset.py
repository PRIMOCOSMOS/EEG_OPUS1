...
def load_index(npz_dir: str | Path) -> pd.DataFrame:
    """Load window index from NPZ directory."""
...
def split_index_by_subjects(df: pd.DataFrame, train_subjects: List[int], val_subjects: List[int]) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Split index by train/val subjects."""
...
def fit_channel_stats(df: pd.DataFrame, max_shards: int = 0) -> tuple[np.ndarray, np.ndarray]:
    """Fit per-channel mean/std on training rows only."""
...
class WindowNpzDataset(Dataset):
    """Dataset for EEG windows from NPZ shards.
    
    FIX: Ensures consistent float32 output for EEG data.
    """
    ...
    def __init__(
        ...
        # FIX: Store as float32 for consistency
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        # FIX: Ensure float32 and proper normalization
        ...
        return {
            "eeg": torch.from_numpy(x).unsqueeze(0).float(),  # Ensure float32
